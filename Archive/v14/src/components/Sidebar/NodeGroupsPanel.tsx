...
export const NodeGroupsPanel: React.FC = () => {
    ...
    // Resolve which nodes match current selections for a group
    ...
    return (
        <div className="flex flex-col">
            {groups.map(group => {
                ...
                const allStepsDone = completedSteps === group.steps.length;
                        ...
                        {isOpen && (
                            <div className="bg-neutral-850 pb-2">
                                ...
                                {group.steps.map((step, stepIdx) => {
                                    const prevStepId = stepIdx > 0 ? group.steps[stepIdx - 1].id : null;
                                    const isPrevDone = prevStepId ? !!groupSel[prevStepId] : true;
                                    const isStepActive = isPrevDone;
                                ...
                                {allStepsDone && (
                                    <div className="mx-2 mt-2">
                                        <p className="text-[10px] text-neutral-500 uppercase tracking-wider mb-1 px-1">
                                            {matchingNodes.length > 0
                                                ? `${matchingNodes.length} нод${matchingNodes.length === 1 ? 'а' : matchingNodes.length < 5 ? 'ы' : ''} — перетащите на холст`
                                                : 'Нет подходящих нод для этой комбинации'}
                                        </p>
                                            ...
                                            {matchingNodes.map(nodeEntry => {
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        className="flex items-center gap-2 p-2 rounded border cursor-grab active:cursor-grabbing transition-all hover:brightness-110"
                                                        style={{
                                                    ...
                                                    >
                                                        ...
                                                        {!def && (
                                                            <span className="text-[9px] text-yellow-600 ml-auto">
                                                                не найдена
                                                            </span>
                                                        )}
                                                        {def && (
                                                            <span className="text-[9px] text-neutral-600 ml-auto truncate max-w-[60px]">
                                                                {def.name}
                                                            </span>
                                                        )}
                                                    </div>
