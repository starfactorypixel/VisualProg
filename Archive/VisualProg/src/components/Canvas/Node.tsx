...
export const Node: React.FC<NodeProps> = ({ data, onContextMenu }) => {
    const {
        nodeDefinitions, updateNodePosition, updateNodesPosition, updateNodeData,
    ...
    const isHoveredNodeMatch = hoveredNodeId && (
        ...
        connections.some(c => {
            ...
            // Simple check: Data nodes never have execution connections
            ...
            // If both are commands, check if the ports are execution ports
            // However, a simpler heuristic: in our system, command-to-command links 
            // are almost always execution flow, and command-to-data are data flow.
            // Let's be explicit and check the port types if possible.
            return false; // For now, assume command-to-command is always execution or better excluded from hover
        })
    );
    ...
    const isLinkingMatch = useEditorStore((state) => {
        ...
        // If we are dragging an output, we look for compatible inputs
    ...
    const handlePortMouseUp = (e: React.MouseEvent, portId: string, _suggestedType: 'input' | 'output') => {
        ...
        if (e.button === 0 && currentLinking && currentLinking.fromNodeId !== data.id) {
            ...
            // Determine Source (Output) and Target (Input) based on the dragging direction
    ...
    useEffect(() => {
        const lastPos = { x: dragStartNodePos.current.x, y: dragStartNodePos.current.y };

        const handleWinMouseMove = (e: MouseEvent) => {
            if (!isDragging) return;
            const { scale, snapToGrid } = useEditorStore.getState();
            const GRID_SIZE = 20;
            ...
            if (selectedNodeIds.includes(data.id)) {
                // If this node is part of the selection, move the whole selection
                updateNodesPosition(delta);
            } else {
    ...
    return (
        <div
            ...
            style={{
                ...
                top: data.position.y,
                borderColor: isHighlighted ? '#fff' : (definition.visual?.color || '#555')
            }}
        ...
        >
                ...
                {data.type === 'data' && (
                    <div className="mb-2">
                        {(() => {
                            ...
                            // Handle Group Selector Logic
                            if (isSelector && groupName) {
                                const store = useEditorStore.getState();
                                const groupValues = store.nodes
                                    .filter(n => n.data?.group === groupName && n.id !== data.id && !n.data?.isGroupSelector)
                                    .map(n => n.data?.value)
                                    .filter(v => v !== undefined && v !== '');

                                const uniqueValues = Array.from(new Set(groupValues));
            ...
            {/* Unified Data Connection Overlays (Root Level - Strictly on Visual Border) */}
            {data.type === 'data' && (
                <>
                    {/* Left hit area (wide) */}
                    <div
