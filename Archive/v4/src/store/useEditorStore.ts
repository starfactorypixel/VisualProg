...
export const useEditorStore = create<EditorStore>((set, get) => ({
    ...
    addNode: (node) => set((state) => {
        ...
        if (state.magneticGridMode) {
            ...
            return { nodes: snapped, gridRows };
        }
        return { nodes: newNodes, gridRows: state.gridRows };
    }),
    ...
    endDragging: (commit) => set((state) => {
        ...
        if (commit && state.draggingState.isMagnetic) {
            ...
            // Возвращаем все выбранные ноды в исходную позицию
            const hasSelection = state.selectedNodeIds.length > 1 && state.selectedNodeIds.includes(state.draggingState.nodeId);
    ...
    removeNode: (id) => set((state) => {
        ...
        if (state.magneticGridMode) {
            ...
            return { nodes: snapped, connections: newConnections, selectedNodeIds: state.selectedNodeIds.filter(idx => idx !== id), gridRows };
        }
        return { nodes: newNodes, connections: newConnections, selectedNodeIds: state.selectedNodeIds.filter(idx => idx !== id), gridRows: state.gridRows };
    }),
    ...
    addConnection: (connection) => set((state) => {
        ...
        if (state.magneticGridMode) {
            ...
            return { connections: newConnections, nodes: snapped, gridRows };
        }
        return { connections: newConnections, gridRows: state.gridRows };
    }),
