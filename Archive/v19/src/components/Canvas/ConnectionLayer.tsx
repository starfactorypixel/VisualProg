...
import { MemoizedFireflyParticles, FireflyConfig } from './FireflyParticles';
...
// Глобальная конфигурация огоньков (настраиваемая)
const DEFAULT_FIREFLY_CONFIG: FireflyConfig = {
    color: '#ffffff',
    particleCount: 2,
    minSpeed: 0.15,
    maxSpeed: 0.3,
...
export const ConnectionLayer: React.FC<ConnectionLayerProps> = ({ 
    ...
    // Track mouse position for temporary connection line - с throttle
    ...
    }, [pan, scale, containerRef]);

    // Виртуализация соединений - только видимые ноды
    ...
    // Виртуализация соединений - рендерим если хотя бы одна нода видна
    const visibleConnections = useMemo(() => {
        return connections.filter(
            conn => visibleNodeIds.has(conn.fromNodeId) || visibleNodeIds.has(conn.toNodeId)
        );
    ...
    // Memoize connection paths calculation - только для видимых соединений
    ...
    }, [visibleConnections, hoveredNodeId, hoveredType, getPortInfo, getPortColor, nodes]);
                ...
                {connectionPaths.map(connPath => {
                    ...
                    return (
                        <g key={connPath.id}>
                            ...
                            />
                            {/* Оптимизированные огоньки с централизованным управлением кадрами */}
                            {fireflyAnimationFPS > 0 && (
                                <MemoizedFireflyParticles
                                    key={connPath.path}
                                    path={connPath.path}
                                    config={{
                                        ...DEFAULT_FIREFLY_CONFIG,
                                        color: connPath.isHighlighted ? '#ffffff' : connPath.color
                                    }}
                                    targetFPS={fireflyAnimationFPS}
                                    paused={false}
                                />
                            )}
                        </g>
