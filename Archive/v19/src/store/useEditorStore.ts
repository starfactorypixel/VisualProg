...
interface EditorStore {
    ...
    // Настройки анимации огоньков
    fireflyAnimationFPS: number;   // Частота обновления анимации (0 = выключено)
...
export const useEditorStore = create<EditorStore>((set, get) => ({
    ...
    // Настройки анимации огоньков по умолчанию (24 FPS)
    fireflyAnimationFPS: 24,
    ...
    // Настройки анимации огоньков
    setFireflyFPS: (fps) => set(() => ({
        fireflyAnimationFPS: Math.max(0, Math.min(60, fps))
    })),
    ...
    updateDragging: (position) => set((state) => {
        ...
        // Оптимизация: не обновляем ноды если позиция не изменилась значительно
        const currentLeadNode = state.nodes.find(n => n.id === state.draggingState!.nodeId);
        if (currentLeadNode && 
            Math.abs(currentLeadNode.position.x - newLeadPosition.x) < 0.1 && 
            Math.abs(currentLeadNode.position.y - newLeadPosition.y) < 0.1) {
            return {
                draggingState: {
                    ...state.draggingState,
                    isMagnetic,
