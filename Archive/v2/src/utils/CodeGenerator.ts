...
export class CodeGenerator {
    ...
    private getNextNode(currentNode: NodeInstance, visited: Set<string>): NodeInstance | undefined {
        ...
        // Normal flow - get ALL outgoing connections, sorted by port order (true before false)
        const execPortOrder = ['exec_out', 'next', 'exec_true', 'true', 'exec_false', 'false'];
        const outConns = this.connections
            .filter(c =>
        ...
        entryPointNodes.forEach(startNode => {
            ...
            while (current && !visited.has(current.id)) {
                ...
                const outConns = this.connections
                    ...
                    .sort((a, b) => execPortIds.indexOf(a.fromPortId) - execPortIds.indexOf(b.fromPortId));
