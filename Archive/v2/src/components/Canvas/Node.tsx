...
export const Node: React.FC<NodeProps> = ({ data, onContextMenu }) => {
    ...
    useEffect(() => {
        const handleWinMouseMove = (e: MouseEvent) => {
            ...
            const store = useEditorStore.getState();
            const { scale, selectedNodeIds, magneticGridMode  } = store;
...
/*
            if (store.magneticGridMode && (store as any).updateDragging) {
                // Магнитный режим для команд
                (store as any).updateDragging({ x: newX, y: newY });
            } else {
                // Обычное перемещение для данных или когда режим выключен
                updateNodePosition(data.id, { x: newX, y: newY });
            }
*/
            // Магнитный режим
            if (magneticGridMode && (store as any).updateDragging) {
                // Обновляем позицию перетаскивания (для расчета магнита)
                (store as any).updateDragging({ x: newX, y: newY });
                
                // Если есть магнитная позиция, применяем её к группе
                const draggingState = (store as any).draggingState;
                if (draggingState?.isMagnetic && draggingState?.magneticPosition) {
                    // Используем метод для группового перемещения
                    if (selectedNodeIds.length > 1 && selectedNodeIds.includes(data.id)) {
                        // Групповое магнитное перемещение
                        (store as any).updateSelectedNodesMagnetic?.(data.id, draggingState.magneticPosition);
                    } else {
                        // Одиночное магнитное перемещение
                        (store as any).updateNodePositionMagnetic?.(data.id, draggingState.magneticPosition);
                    }
                }
            } else {
