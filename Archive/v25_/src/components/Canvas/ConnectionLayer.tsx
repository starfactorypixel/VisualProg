...
export const ConnectionLayer: React.FC<ConnectionLayerProps> = ({ onContextMenu }) => {
    ...
    // Refs to visible SVG path elements — shared с DragManager для ghost drag
    const visPathRefs = useRef<Map<string, SVGPathElement>>(DragManager.pathRefs);

    // Регистрируем calcPath в DragManager один раз
    React.useEffect(() => {
        DragManager.calcPath = calcConnectionPath;
        DragManager.pathRefs = visPathRefs.current;
    }, []);
    ...
    React.useEffect(() => {
        ...
        const handleMouseUp = () => setMousePos(null);
                    ...
                    return (
                        <g key={connPath.id}>
                            ...
                            <path
                                ref={el => {
                                    if (el) {
                                        visPathRefs.current.set(connPath.id, el);
                                        DragManager.pathRefs.set(connPath.id, el);
                                    } else {
                                        visPathRefs.current.delete(connPath.id);
                                        DragManager.pathRefs.delete(connPath.id);
                                    }
                                }}
                                d={connPath.path}
                                stroke={strokeColor}
