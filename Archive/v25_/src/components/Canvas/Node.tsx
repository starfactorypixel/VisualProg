...
export const NodeComponent: React.FC<NodeProps> = ({ data, onContextMenu }) => {
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
        ...
        if (e.button === 0) {
            ...
            // Определяем список двигающихся нод
            const movingIds = store.selectedNodeIds.length > 1 && store.selectedNodeIds.includes(data.id)
                ? store.selectedNodeIds
                : [data.id];

            // Стартуем DragManager — он кеширует DOM-элементы и связи один раз
            DragManager.start(
                data.id,
                movingIds,
            ...
            // Настраиваем onMove для магнитного snap — ПОСЛЕ start(), т.к. start() сбрасывает onMove
            if (store.magneticGridMode) {
                DragManager.onMove = (rawDx, rawDy) => {
                    const newX = dragStartNodePos.current.x + rawDx;
                    const newY = dragStartNodePos.current.y + rawDy;
                    const nearest = useEditorStore.getState().findNearestGridPoint({ x: newX, y: newY }, data.type);
                    const snapping = nearest !== null && nearest.distance <= useEditorStore.getState().magneticGridConfig.magnetRadius;

                    // Обновляем только isMagnetic ring — без nodes
                    const ds = useEditorStore.getState().draggingState;
                    if (ds) {
                    ...
                    if (snapping) {
                        return {
                            dx: nearest!.point.x - dragStartNodePos.current.x,
                            dy: nearest!.point.y - dragStartNodePos.current.y,
                            isMagnetic: true
                        };
                    }
                    return { dx: rawDx, dy: rawDy, isMagnetic: false };
                };
            }

            // startDragging для магнитного ring
    ...
    useEffect(() => {
        const handleWinMouseMove = (e: MouseEvent) => {
            ...
            // Просто передаём смещение — DragManager сам применит в rAF
            DragManager.update(rawDx, rawDy);
        };
        ...
        const handleWinMouseUp = () => {
            ...
            const store = useEditorStore.getState();
            const { dx: odx, dy: ody } = DragManager.stop();
            ...
            if (store.magneticGridMode) {
                // Магнитный режим: делегируем endDragging — он знает про originalPosition и snap
                const isMagnetic = store.draggingState?.isMagnetic ?? false;
                if (isMagnetic && store.draggingState?.magneticPosition) {
                    // Записываем snap-позицию в store ДО вызова endDragging,
                    // чтобы recalculateAndSnap внутри него работал от правильной точки
                    const snapPos = store.draggingState.magneticPosition;
                    useEditorStore.setState((state) => ({
                        nodes: state.nodes.map(n =>
                            n.id === data.id ? { ...n, position: snapPos } : n
                        )
                    }));
                }
                store.endDragging(isMagnetic);
            } else {
                // Обычный режим: коммитим финальные позиции одним setState
                useEditorStore.setState((state) => ({
                    draggingState: null,
                    nodes: state.nodes.map(n => {
