...
/**
 * Раскладывает произвольные command-ноды (напр. только что импортированные
 * дизассемблером) по реальным линиям магнитной сетки — тем же rowY/columnX,
 * что использует ручное перетаскивание (findNearestGridPoint), учитывая
 * фактическую высоту ряда (кол-во data-портов у команд в нём). Row-группировка
 * нод сохраняется (кластеризация по Y с допуском 50px в recalculateGridRows),
 * меняются только точные координаты.
 */
